...
#        print("Total face Marked:",j)
 ...
 #               print(j,start, k," faces", len(hEdgeList)," edges")
    ...
    def CheckSegmentation(self):
        # Check Segmentation Color
        smoothData = vtk.vtkPolyData()
        smoothData = CyMesh.CGAL_Poly3_Remesh(self.data, 0.8)

        #segment = vtk.vtkUnsignedIntArray()
        segment = vtk.vtkDoubleArray()
        segment = CyMesh.CGAL_SM_Proto(smoothData, 0.3, (2.0 / 3.0) * math.pi, 10)  # smoothing_lamda, coneAngle, nSegment
        for i in range(smoothData.GetNumberOfPolys()):
            d = segment.GetTuple1(i)
            segment.SetTuple1(i,d*255.0)
            if (i%10 == 0): print(i,d,d*255.0)
        smoothData.GetCellData().SetScalars(segment)
        ...
        for i in range(nColors):
            ...
            print(nColors,i,d1,d2,d3)

        self.segMapper = vtk.vtkPolyDataMapper()
        self.segMapper.SetInputData(smoothData)
        self.segMapper.SetLookupTable(lut)
        self.segMapper.SetScalarRange(0.0,1.0)

        # Create scalar bar
        self.scalarBar = vtk.vtkScalarBarActor()
        self.scalarBar.SetLookupTable(self.segMapper.GetLookupTable())
        self.scalarBar.SetNumberOfLabels(11)

        self.segActor = vtk.vtkActor()
        self.segActor.SetMapper(self.segMapper)
        self.ren.AddActor(self.segActor)
        self.ren.AddActor(self.scalarBar)

        #segColors = vtk.vtkUnsignedCharArray()
        #segColors.SetNumberOfComponents(3)
        #segColors.SetName("Colors")

        #for i in range(smoothData.GetNumberOfPolys()):
        #    n = int(segment.GetValue(i))
        #    self.Colors.SetComponent(i, 0, n)
        #    self.Colors.SetComponent(i, 1, n)
        #    self.Colors.SetComponent(i, 2, n)

#            if n == 0 : # bright pink
#                segColors.InsertNextTuple3(255,153,204)
#            elif n == 1: # bright orange
#                segColors.InsertNextTuple3(255,204,153)
#            elif n == 2: # light green
#                segColors.InsertNextTuple3(204,255,153)
#            elif n == 3: # grey
#                segColors.InsertNextTuple3(128,128,128)
#            elif n == 4: # blue
#                segColors.InsertNextTuple3(0,0,128)
#            elif n == 5: # red
#                segColors.InsertNextTuple3(128,0,0)
#            elif n == 6: # green
#                segColors.InsertNextTuple3(0,128,0)
#            elif n == 7:  # RG =
#                segColors.InsertNextTuple3(128,128,0)
#            elif n == 8:  # GB
#                segColors.InsertNextTuple3(0,128,128)
#            elif n == 9:  # BR
#                segColors.InsertNextTuple3(128,0,128)
#            else:
#                print("n is over 9", i, n)

        #smoothData.GetCellData().SetScalars(segColors)
        self.renWin.Render()
...
#        self.iren.Render()
    ...
    def markVisibleFace(self, idxCaller):
        ...
        # get center
        print("FocalPoint:", fp, "Position", pos, "View Vector:", normal_vec)

        normalizedX = [.0, .0, .0]
        normalizedY = [.0, .0, .0]
        normalizedZ = [.0, .0, .0]
        normalizedZ = [-normal_vec[i] for i in range(len(normal_vec))]
...
#        print(tempData.GetBounds())
        ...
        if hasattr(self,'extrudedData') == False:
            ...
            beginTime = time.time()
            self.extrudedData = CyMesh.Pagoda_Proto(transformPD3.GetOutput(),normal_vec[0],normal_vec[1],normal_vec[2])
            print("Blockout Calc Time", time.time() - beginTime)
...
#        beginTime=time.time()
#        self.MarkOccludedFaces(rPoint1)
...
#                self.Colors.SetComponent(i, 2, 128)
    ...
    def offsetModel(self):
        # stop contourWidget, Using LDNI offset
